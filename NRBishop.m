function [I_cell] = NRBishop(I_ph, I_o, A, R_s, R_sh, a, m, V_br, V_T, V_cell)
    % Se define la ecuación.
    syms I_cell
    
    % SDM
    eq = I_ph - I_o*(exp((V_cell + I_cell*R_s)/(A*V_T)) - 1) - ((V_cell + I_cell*R_s)/R_sh)*(1 + a*(1 - (V_cell + I_cell*R_s)/V_br)^(-m)) - I_cell;
